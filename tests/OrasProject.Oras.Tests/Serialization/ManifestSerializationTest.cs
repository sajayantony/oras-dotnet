...
public class ManifestSerializationTest
{
        ...
        string json, bool isIndex)
    {
        ...
        if (isIndex)
        {
            var idx = OciJsonSerializer
                .Deserialize<OciIndex>(bytes)!;
            reserialized =
                OciJsonSerializer.SerializeToUtf8Bytes(idx);
        }
        else
        {
            var m = OciJsonSerializer
                .Deserialize<Manifest>(bytes)!;
            reserialized =
        ...
        if (isIndex)
        {
            var orig = OciJsonSerializer
                .Deserialize<OciIndex>(bytes)!;
            var got = OciJsonSerializer
                .Deserialize<OciIndex>(reserialized)!;
            Assert.Equal(
                orig.Manifests.Count, got.Manifests.Count);
        }
        else
        {
            var orig = OciJsonSerializer
                .Deserialize<Manifest>(bytes)!;
            var got = OciJsonSerializer
                .Deserialize<Manifest>(reserialized)!;
